...
mp.events.add("playerJoin", (player) => {
    ...
    player.sendToCreator = function() {
        ...
        player.call("gangwartoggleCreator", [true, JSON.stringify(player.customCharacter)]);
    ...
    player.sendToWorld = function() {
        ...
        player.changedGender = false;
        player.call("gangwartoggleCreator", [false]);
    };

    player.loadCharacter();
});

mp.events.add("gangwar_creator_GenderChange", (player, gender) => {
...
mp.events.add("gangwar_creator_Save", (player, gender, parentData, featureData, appearanceData, hairAndColorData) => {
    ...
    player.call("customEvalWithDelay", [`openTeamUI`, 500])
});

mp.events.add("gangwar_creator_Leave", (player) => {
    if (player.changedGender) player.loadCharacter(); // revert back to last save if gender is changed
    player.sendToCreator();
});

mp.events.add("server:loadcharacter", (player) => {
    player.loadCharacter();
    player.call("customEvalWithDelay", [`openTeamUI`, 2500])
});

mp.events.add("server:opencharcreator", (player) => {
