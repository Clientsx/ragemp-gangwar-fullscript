...
creatorMainMenu.ListChange.on((item, listIndex) => {
    if (item == genderItem) {
        ...
        mp.events.callRemote("gangwar_creator_GenderChange", listIndex);
...
creatorMainMenu.ItemSelect.on((item, index) => {
    switch (index) {
        ...
        case 6:
            ...
            mp.events.callRemote("gangwar_creator_Save", currentGender, JSON.stringify(parentData), JSON.stringify(featureData), JSON.stringify(appearanceData), JSON.stringify(hairAndColors));
        break;

        case 7:
            mp.events.callRemote("gangwar_creator_Leave");
        break;
    }
});
...
// EVENTS
mp.events.add("gangwartoggleCreator", (active, charData) => {
    if (active) {
